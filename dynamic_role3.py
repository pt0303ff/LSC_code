...
# ============ 0) 你的既有資源 ============
CSV_PATH = "0313moli_comments_parsed4_noL.csv"
SVM_PATH = "0316bge_m3_calibrated_linear_svc_kocfkos.joblib"
...
def svm_predict(text: str) -> Dict[str, Any]:
    ...
    # calibrated SVM 可以輸出各類別機率
    proba = svm_model.predict_proba(emb)[0]
    ...
    top1_role = classes[top1_idx]
    top1_prob = float(proba[top1_idx])

    top2_role = classes[top2_idx]
    ...
    return {
        ...
        "top2_prob": top2_prob,
        "prob_margin": margin,
        "all_probs": dict(zip(classes, [float(p) for p in proba]))
    }
...
SVM_CONF_THRESHOLD = 0.70                 # 低於這個值才呼叫 LLM
SVM_MARGIN_THRESHOLD = 0.20               #機率相差小於0.2也呼叫LLM

ROLE_DEFS = {
    "KOF": "追隨型觀眾。留言中表現對直播主的支持、熟悉、問候、稱讚、固定互動或聊天，但沒有明確購買意圖，也沒有分享使用經驗。",
    "KOC": "購買型觀眾。留言中具有明確交易或購買意圖，例如下單、+1、詢價、問尺寸、問價格、問庫存、幫我留、我要、怎麼買、可以訂嗎。",
    "KOS": "分享傳播型觀眾。留言中出現購買後心得、使用經驗、推薦他人、開箱、回購、宣傳直播、鼓勵他人購買、分享產品效果、替產品背書或分享此直播。",
    "other": "其他。無法判斷角色、自己與其他觀眾聊天與當下直播內容無關、無明確意圖、純表情、無意義文字、一般模糊回應、或不符合 KOC/KOF/KOS 的留言。"
}
...
def run(out_csv="transitions9.csv", history_window=6):
    ...
    for row in df.itertuples(index=False):
       ...
       # 1) 先跑 SVM（靜態）
        ...
        prob_margin = svm_out["prob_margin"]
        
        need_llm = (
            svm_conf < SVM_CONF_THRESHOLD or
            prob_margin < SVM_MARGIN_THRESHOLD
        )
        ...
        if not need_llm:
            ...
            llm_out = {
                ...
                "reason": f"SVM高信心直接採用(conf={svm_conf:.2f}, margin={prob_margin:.2f})"
            }
        else:
            used_llm = True
            try:
        ...
        # 3) 記錄角色轉移
        if llm_out.get("changed") is True and st.current_role is not None:
            evt = {
                "user": user,
                "time": t,
                "sec": sec,
        ...
        all_predictions.append({
            ...
            "top1_role": svm_out["top1_role"],
            "top1_prob": svm_out["top1_prob"],
            "top2_role": svm_out["top2_role"],
            "top2_prob": svm_out["top2_prob"],
        ...
        pd.DataFrame(all_predictions).to_csv("all_predictions9.csv",index=False,encoding="utf-8-sig")
    ...
    print("Saved: role_counts.csv, user_roles_summary.csv, all_pridiciotns.csv")
...
if __name__ == "__main__":
    ...
    run(out_csv="transitions9.csv", history_window=6)

    end_time = time.time()
    print(f"執行{end_time-start_time}秒")


#transitions6.csv CalibratedCV機率後 gpt-oss:latest
#transitions7.csv CCV gemma4:e4b
#transitions8.csv CCV qwen3.5:9b
#transitions9.csv CCV gemma4:26b